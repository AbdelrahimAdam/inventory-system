...
const RegisterPage: React.FC = () => {
  ...
  return (
    <div dir="rtl" className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-900 dark:to-gray-800 px-4 relative overflow-hidden">
  ...
  <h2 className="text-2xl font-bold mb-6 text-center text-gray-800 dark:text-white">إنشاء حساب جديد</h2>
        ...
        {error && <div className="mb-4 text-red-500 text-sm text-center animate-pulse">{error}</div>}
        {success && <div className="mb-4 text-green-500 text-sm text-center animate-fade">{success}</div>}

        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-200">الاسم الكامل</label>
            <input
              type="text"
          ...
          <div>
            <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-200">كلمة المرور</label>
          ...
          <div>
            <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-200">اختر الدور</label>
            <select
              value={role}
          ...
          <button type="submit" className="w-full py-2 bg-green-600 hover:bg-green-700 text-white rounded-md transition">
            إنشاء الحساب
          </button>

          <p className="text-sm text-center text-gray-600 dark:text-gray-400 mt-4">
            لديك حساب؟ <a href="/login" className="text-blue-600 hover:underline">سجل الدخول</a>
          </p>
        </form>
      </div>
    </div>
