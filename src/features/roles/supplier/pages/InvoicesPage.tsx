// src/features/roles/supplier/pages/InvoicesPage.tsx
...
interface Invoice {
  ...
  invoice_type: 'PURCHASE' | 'PURCHASE_RETURN' | 'SALE' | 'SALE_RETURN' | 'FACTORY_DISPATCH';
  invoice_date: string;
...
const InvoicesPage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Fetch invoices
      const invoicesResponse = await fetch('/api/invoices/supplier', {
        headers: {
      ...
      await logSecurityEvent('INVOICES_VIEW', 'تم عرض قائمة فواتير المورد', true);
    } catch (err: any) {
      setError(err.message || 'حدث خطأ أثناء تحميل البيانات');
      await logSecurityEvent('INVOICES_VIEW', `فشل عرض فواتير المورد: ${err.message}`, false);
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    try {
      await fetch('/api/security/log', {
        ...
        body: JSON.stringify({
          ...
          details: { component: 'SupplierInvoices' }
        })
      });
    } catch (error) {
  ...
  const getInvoiceTypeLabel = (type: string): string => {
    switch (type) {
      ...
      case 'FACTORY_DISPATCH': return 'تحويل مصنع';
      default: return type;
    }
  };
  ...
  const getInvoiceTypeColor = (type: string): string => {
    switch (type) {
      ...
      case 'SALE_RETURN': return 'text-amber-600 dark:text-amber-400';
      case 'FACTORY_DISPATCH': return 'text-purple-600 dark:text-purple-400';
  ...
  const validateInvoice = (data: InvoiceFormData): string | null => {
  ...
  const handleExportInvoices = async () => {
    try {
      ...
      setSuccess('تم تصدير الفواتير بنجاح');
      
      await logSecurityEvent('DATA_EXPORT', 'تم تصدير فواتير المورد', true);
    } catch (err: any) {
      setError(err.message || 'حدث خطأ أثناء التصدير');
      await logSecurityEvent('DATA_EXPORT', `فشل تصدير فواتير المورد: ${err.message}`, false);
    } finally {
      setExporting(false);
    }
  };
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-6xl w-full max-h-[90vh] overflow-hidden">
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-700 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600">
                    <span className="text-gray-900 dark:text-white">
                      {selectedInvoice.supplier_name || 'غير محدد'}
                    </span>
                  </div>
                </div>
            ...
            {/* Items Table */}
            <h4 className="font-semibold text-gray-900 dark:text-white mb-4">عناصر الفاتورة</h4>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                {errors.invoice_type && (
                  <p className="mt-1 text-sm text-red-600 dark:text-red-400">{errors.invoice_type.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                {errors.supplier_name && (
                  <p className="mt-1 text-sm text-red-600 dark:text-red-400">{errors.supplier_name.message}</p>
                )}
              </div>
            </div>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                <h4 className="font-semibold text-gray-900 dark:text-white">عناصر الفاتورة</h4>
                <button
                  type="button"
                  ...
                  {items.map((item, index) => (
                    <div key={index} className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg border border-gray-200 dark:border-gray-600">
                      <div className="flex items-center justify-between mb-3">
                        <h5 className="font-medium text-gray-900 dark:text-white">عنصر #{index + 1}</h5>
                        <button
                          type="button"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                          ...
                          >
                            ...
                            {inventoryItems.filter(item => item.is_active).map(invItem => (
            ...
            {items.length > 0 && (
              <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <div className="flex justify-between items-center">
                  <span className="text-blue-900 dark:text-blue-300 font-medium">الإجمالي:</span>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                  {filteredInvoices.map((invoice) => (
                    <tr key={invoice.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="py-4 px-4">
                        <div className="text-right">
                          <div className="text-gray-900 dark:text-white">{invoice.supplier_name || 'غير محدد'}</div>
                        </div>
                      </td>
                      <td className="py-4 px-4">
